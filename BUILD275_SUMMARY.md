...
- The deeper `/maintenance-plan` waitlist also had a broken `#cycleSummary` initialization reference and stale fixed-plan defaults such as “Every 4 or 8 weeks” and benefit-style promises. The **maintenance waitlist authority** now removes that broken DOM write, keeps the page explicitly waitlist-only, and sanitizes public growth settings so retained App Management experiments cannot republish unapproved cadence, pricing, discounts or perks.
- Build 275 source CI now syntax-checks both public growth-settings modules in addition to the booking and vehicle-catalogue modules.
...
The focused Build 275 guard pins the utility compatibility bridge, canonical slot projection, returning-customer prefill boundary, funnel-exit suppression/privacy rules, existing bounded analytics authority, retained vehicle-catalogue fail-open behavior, published add-on route integrity, fleet utility authority, maintenance utility authority, maintenance waitlist authority, and the Build 275 Development acceptance boundary. The Build 275 feature source gate also reruns the cumulative release guard plus retained Builds 271, 272, 273 and 274 guards.
...
3. Continue fleet/workplace planning without inventing undecided pricing, perks or vehicle-count minimums; fleet and maintenance utility wording and the maintenance waitlist contract are now aligned while commercial/recurring economics remain intentionally undecided.
