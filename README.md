...
Use `DOC_INDEX.md` only to locate specialist references. The completed `FORWARD_BUILD_ROADMAP_378_385.md` and `docs/BACKUP_RESTORE_RELEASE_RECOVERY.md` remain retained historical/specialist authorities, not the normal restart point.
...
Run the focused current baseline/roadmap authority with:

```bash
python scripts/post_recovery_baseline_roadmap_check.py
```
