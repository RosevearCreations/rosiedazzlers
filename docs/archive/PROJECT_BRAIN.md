...
## Build 183 project brain note

The payment system now has a practical office workflow: provider webhooks settle payments, staff can replay verified events, refunds can be recorded manually or initiated through Stripe/PayPal, receipts/refund emails are queued, and reconciliation can be exported as CSV. Image readiness is now tracked through `IMAGES.md` and `data/image_requirements_build183.json`.
...
## Build 193 project memory

Admin Social template options are DB-first but must never break manual drafting. `/api/admin/social_templates_list` now safely handles omitted filters and null fallback row fields. Editable settings now have bundled validation schemas and warn about unsupported template tokens and navigation/footer link problems before save.
