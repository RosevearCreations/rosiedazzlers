...
Use `/admin-leads.html` to test: build quote starter, save quote draft, create conversion draft, load conversion drafts, then create a real booking after entering date/slot/address/package/vehicle/pricing details. Use `/admin-analytics.html` for conversion summary and `/admin-app.html` for media privacy readiness.
...
Use the latest zip. Apply Build 180 SQL after Build 179. Test `/admin-leads.html` and `/quote-payment.html`. Next build should wire Stripe/PayPal webhook events into `quote_deposit_payment_requests` so payments can be marked paid automatically.
---
...
Latest build: Build 183. Focus areas completed: direct provider refunds, payment reconciliation export, webhook warning dashboard, and rebuilt image requirements. Next chat should verify uploaded media, test payment refund sandbox flows, and add R2 image-health scanner/reporting.
