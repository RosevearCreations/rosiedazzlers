...
create table if not exists public.bookings (
  ...
  status text not null default 'pending', -- pending|confirmed|cancelled|completed (keep consistent with code)
  ...
  -- token-based progress system (already in repo functions/api/progress/view.js)
  progress_token uuid null unique,
  progress_enabled boolean not null default false,
  job_status text null, -- scheduled|in_progress|completed etc (used by admin/booking_update.js)
...
-- (Keep RLS locked down; service role is used by serverless APIs.)


-- -----------------------------
-- DATE BLOCKS (full-day)
-- -----------------------------
...
-- -----------------------------
-- GIFT PRODUCTS (catalog of SKUs sold)
-- -----------------------------
create table if not exists public.gift_products (
  id uuid primary key default gen_random_uuid(),
  sku text not null unique,
  type text not null, -- open_value | service | amount_value (as your code expects)
  name text not null,
...
-- -----------------------------
-- GIFT CERTIFICATES (issued on webhook)
-- -----------------------------
create table if not exists public.gift_certificates (
  ...
  purchase_context jsonb null, -- stores intake details as in webhook
...
-- -----------------------------
-- BOOKING EVENTS (audit log for Stripe webhook)
-- -----------------------------
...
-- -----------------------------
-- SIMPLE PROGRESS UPDATES (working now)
-- -----------------------------
...
-- -----------------------------
-- TOKEN-BASED PROGRESS SYSTEM (more secure, partially wired in repo)
-- -----------------------------
...
create table if not exists public.job_media (
  ...
  created_at timestamptz not null default now(),
  created_by text null,
  kind text null, -- photo|video
  caption text null,
  media_url text not null, -- can store sb://bucket/path or https://...
  visibility text not null default 'customer' check (visibility in ('customer','internal'))
);
