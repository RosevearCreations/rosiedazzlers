...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!payload.item_key || !payload.name || !['tool','consumable'].includes(payload.item_type)) {
      return withCors(json({ error: 'Missing required fields.' }, 400));
    }
    if (!['reorder','single_use','never_reuse'].includes(payload.reuse_policy)) {
      return withCors(json({ error: 'Invalid reuse policy.' }, 400));
    }

    const saved = await upsertInventoryItem(env, payload);
    if (!saved.ok) return withCors(json({ error: saved.error }, 500));

    return withCors(json({
      ok: true,
      item: saved.item || null,
      stripped_columns: saved.stripped_columns || []
    }));
  } catch (err) {
...
async function upsertInventoryItem(env, payload) {
  let working = { ...payload };
  const stripped = [];
  for (let attempt = 0; attempt < OPTIONAL_COLUMNS.length + 1; attempt += 1) {
    ...
    if (res.ok) {
      ...
      return { ok: true, item: Array.isArray(rows) ? rows[0] || null : null, stripped_columns: stripped };
    }
    const errorText = await res.text();
    const missing = OPTIONAL_COLUMNS.find((column) => working[column] !== undefined && mentionsMissingColumn(errorText, column));
    if (!missing) return { ok: false, error: errorText };
    delete working[missing];
    stripped.push(missing);
  }
  return { ok: false, error: 'Could not save inventory item after compatibility fallback.' };
}

function mentionsMissingColumn(errorText, column) {
