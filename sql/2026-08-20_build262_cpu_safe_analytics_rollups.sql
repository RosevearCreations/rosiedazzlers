...
create or replace function public.refresh_site_activity_rollups_cpu_safe(
...
begin
  -- Calculate the bounded reporting window once.
  v_since := now() - make_interval(days => v_days);
  v_start_date := v_since::date;
  ...
  delete from public.site_activity_rollups
  where period_end >= v_start_date;

  delete from public.site_activity_dimension_daily_rollups
  where rollup_date >= v_start_date;

  delete from public.site_activity_funnel_daily_rollups
  where rollup_date >= v_start_date;
  ...
  with scoped as (
    ...
    cross join lateral (
      ...
      select coalesce(
        nullif(btrim(e.payload->>'service_area_label'), ''),
        nullif(btrim(e.payload->>'service_area'), '')
      )
      where coalesce(
        nullif(btrim(e.payload->>'service_area_label'), ''),
        nullif(btrim(e.payload->>'service_area'), '')
    ...
    cross join lateral (
      values
        ...
        (
          ...
          (
            date_trunc('week', scoped.created_utc)
            + interval '6 days'
          )::date
        ),
        (
          ...
          (
            date_trunc('month', scoped.created_utc)
        ...
        (
          ...
          (
            date_trunc('year', scoped.created_utc)
  ...
  select
    ...
    count(*) filter (
      where event_type = 'checkout_started'
         or checkout_state = 'started'
    )::integer,
    count(*) filter (
      where event_type = 'checkout_completed'
         or checkout_state = 'completed'
    )::integer,
    count(*) filter (
      where event_type = 'cart_snapshot'
    )::integer,
    now()
  from periods
  group by
  ...
  scoped as (
    ...
    cross join lateral (
      ...
      select base.event_area
      where base.event_area is not null
    ) s
  ),
  dims as (
    select
    ...
    cross join lateral (
      values
        ...
        (
          'city'::text,
          case
            when nullif(payload->>'city', '') is null
              then 'Unknown'
            else
              (payload->>'city')
              ||
              case
                when nullif(payload->>'region', '') is null
                  then ''
                else ', ' || (payload->>'region')
              end
          end
  ...
  select
    ...
    count(*) filter (
      where event_type = 'booking_step_view'
        and (
          case
            when coalesce(payload->>'step_number', '') ~ '^\d+$'
              then (payload->>'step_number')::integer
            else 0
          end
        ) = 1
    )::integer,
    ...
    count(*) filter (
      where event_type = 'booking_service_area_pick'
    )::integer,

    count(*) filter (
      where event_type = 'booking_date_pick'
    )::integer,

    count(*) filter (
      where event_type = 'booking_package_pick'
    )::integer,

    count(*) filter (
      where event_type = 'booking_addon_toggle'
    )::integer,

    count(*) filter (
      where event_type = 'booking_customer_continue'
    )::integer,
...
revoke all
on function public.refresh_site_activity_rollups_cpu_safe(integer)
