...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const insertRes = await fetch(`${env.SUPABASE_URL}/rest/v1/job_updates`, {
      ...
      body: JSON.stringify([{ booking_id: resolvedBookingId, created_by: actor.full_name || actor.email || cleanText(body.created_by) || "Staff", note, visibility, staff_user_id: actor.id || null }])
    });
