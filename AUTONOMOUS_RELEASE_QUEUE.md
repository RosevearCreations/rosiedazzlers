...
- re-baseline the Development/go-live blocker document so it no longer points to obsolete Build 268-era planning authority;
- retire Build 385 recovery-only workflow coupling from normal `dev`/`main` pushes while preserving the recovery runbook and retained rollback/recovery authorities;
- establish a focused Build 386 source authority that fails closed if accepted/current/next release state, roadmap ownership or release rules drift;
