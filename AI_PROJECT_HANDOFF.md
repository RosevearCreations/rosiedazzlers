...
- **Build 390 — Booking, Quote & Condition-Based Estimate Hardening** is the active bounded release. It separates fixed catalog quotes from inspection-led estimates and keeps quote acceptance, availability, deposit/payment evidence and booking confirmation as distinct server-authoritative states.
- **Build 391 — Photo Studio & R2 Media Reliability** is next only after current-release acceptance is complete.
- The current booking/quote release is schema-neutral. It does not authorize a database migration, database restore, R2 write/delete, DNS mutation, secret rotation, customer charge/refund, provider mutation or Production business-data mutation.
...
- A condition-sensitive service is an estimate/inspection path until its actual scope is reviewed; do not silently convert a starting price into a fixed bookable quote.
- Quote acceptance records customer approval of the delivered commercial snapshot only. It does not prove current slot availability, deposit/payment settlement or a confirmed appointment.
- A successful availability check means available-but-unheld until booking authority creates/holds the appointment. Payment and provider evidence remain separate from final booking status.
- Customer-facing package prices continue to use the accepted commercial authority rather than creating a parallel pricing ledger.
...
- `.github/workflows/booking-quote-estimate-hardening-authority.yml` — focused current quote/estimate/deposit/booking continuity authority.
- `functions/api/_lib/commercial-continuity.js` and `scripts/build390_booking_quote_estimate_hardening_test.mjs` — fail-closed commercial-state semantics and regression proof.
- `.github/workflows/service-commercial-accuracy-authority.yml` — retained pricing and condition-scope authority.
- `.github/workflows/local-seo-service-landing-proof-authority.yml` — retained public local/service/proof authority.
