...
Current contract: BUILD504_SERVICE_ECONOMICS_SEASONAL_CAPACITY_RELIABILITY_TREND_CONTINUITY.md. Current source authority: scripts/service_economics_seasonal_capacity_reliability_trend_continuity_check.py. Current focused workflow: .github/workflows/service-economics-seasonal-capacity-reliability-trend-continuity-authority.yml. Retained Build 494 reconciliation authority: BUILD494_SERVICE_ECONOMICS_SEASONAL_OPERATIONS_RELIABILITY_REVIEW.md. Retained Build 503 staff/mobile authority: BUILD503_STAFF_MOBILE_REMEDIATION_OUTCOME_INTERPRETATION_FOLLOW_UP.md. Retained provider/local-search outcome authority: BUILD499_PROVIDER_LOCAL_SEARCH_OUTCOME_EVIDENCE_REFRESH.md. Retained recovery/device execution contract: BUILD500_RECOVERY_DRILL_AUTHENTICATED_DEVICE_OBSERVATION_EXECUTION_EVIDENCE.md. Retained recovery/device execution source authority: scripts/recovery_authenticated_device_observation_execution_evidence_check.py. Retained maintenance/fleet continuity authority: BUILD501_MAINTENANCE_FLEET_PILOT_OUTCOME_CONTINUITY_REVIEW.md. Retained maintenance/fleet continuity source authority: scripts/maintenance_fleet_pilot_outcome_continuity_review_check.py. Retained booking/quote interpretation authority: BUILD502_BOOKING_QUOTE_EXPERIMENT_OUTCOME_INTERPRETATION.md. Retained booking/quote interpretation source authority: scripts/booking_quote_experiment_outcome_interpretation_check.py. Active roadmap: FORWARD_BUILD_ROADMAP_496_505.md. Canonical HOLD backlog: STARTUP_GO_LIVE_BLOCKERS.md.

## Current Service Economics, Seasonal Capacity & Reliability Trend Continuity contract
- Reuse the retained Build 494 cross-domain reconciliation and owning economics, seasonal, capacity and reliability authorities; do not create a second ledger, weather engine, capacity model, provider telemetry source, recovery engine or trend store.
...
- Southern Ontario seasonal continuity preserves only source-owned cold-snap-capable, temperature-limited-outdoor and controlled-environment-required classifications. Exact temperature limits remain source-owned; broad winter availability is never inferred.
- Observed-capacity continuity requires repeated live observations with the same metric and unit. Demand, revenue, historical work and technical activity are not capacity proxies; future capacity is never forecast.
- Technical reliability continuity reuses the bounded first-party comparison only. It never establishes provider billing/CPU/quota, recovery success, scaling need, field operability or causal explanation.
- Missing comparable history remains explicitly insufficient. Evidence from one domain never closes another.
- Keep the release read-only: no allocation, price/discount, booking/availability, winter claim, capacity, scaling, provider, restore, accounting/inventory, schema/storage, outreach or polling mutation.
- Exact feature, Development, protected-main and Production deployment/runtime/business acceptance remain required.

## Retained Staff & Mobile Remediation Outcome Interpretation & Follow-Up contract
- Reuse the retained protected staff/mobile workbench and retained outcome-evidence layer; do not create a second remediation, telemetry or outcome system.
- Interpret only retained outcome-evidence rows that are already attributable, materially like-for-like and explicitly weather/site classified.
...
- A triggered locked stop condition blocks ordinary interpretation and requires explicit owner review.
- Comparable numeric primary-metric observations are required for every authorized allocation arm. Missing or mismatched values remain fail-closed.
- Per-arm counts, means, minimums and maximums are descriptive only. Threshold evaluation, success and winner selection remain explicit owner review.
- Keep the release read-only: no price/discount, booking-rule/availability, booking, outreach, provider/business, schema/storage or polling mutation.
- Exact feature, Development, protected-main and Production deployment/runtime/business acceptance remain required.

## Retained Maintenance & Fleet Pilot Outcome Continuity Review contract
...
- Reuse the retained staff/mobile endpoint and /admin-staff-workflow-support-learning.html workbench; do not create a parallel telemetry, staff, support or remediation system.
