...
- **Build 389 — Local SEO, Service Landing & Proof Convergence** is the active bounded release. It converges established town/service landings on truthful Oxford/Norfolk SEO, current pricing/catalog authority and fail-closed genuine proof rather than adding duplicate pages or sample evidence.
- **Build 390 — Booking, Quote & Condition-Based Estimate Hardening** is next only after current-release acceptance is complete.
- The current local-SEO/proof release is schema-neutral. It does not authorize a database migration, database restore, R2 write/delete, DNS mutation, secret rotation, customer charge/refund, provider mutation or Production business-data mutation.
...
- Customer-facing package prices continue to use the accepted commercial authority; local/service pages do not create a parallel pricing ledger.
- Landing proof is genuine only when existing Gallery authority marks it published, public-use/privacy approved, non-sample and context-complete. Missing proof fails closed instead of falling back to sample media or invented reviews.
...
- `.github/workflows/local-seo-service-landing-proof-authority.yml` — focused current local SEO/service/proof authority.
- `scripts/build389_local_seo_service_landing_proof_check.py` and `scripts/build389_local_seo_service_landing_proof_test.mjs` — metadata/H1/canonical, current-pricing, retired-link and fail-closed proof guards.
- `.github/workflows/service-commercial-accuracy-authority.yml` — retained commercial/pricing authority inherited by the current public landing flow.
- `.github/workflows/review-proof-local-seo-authority.yml` — retained approved-review/completed-booking/proof opportunity authority.
