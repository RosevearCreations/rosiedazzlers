...
- **Build 387 — Release Governance & Branch Protection Readiness** is the active bounded release. It makes release governance release-number independent, recognizes the active `rd main protection` ruleset, distinguishes stale/advisory checks from current authority, and documents fail-closed operator recovery.
- **Build 388 — Service, Add-On & Commercial Accuracy Convergence** is next only after current-release acceptance is complete.
- The current governance release is schema-neutral. It does not authorize a database migration, database restore, R2 write/delete, DNS mutation, secret rotation, customer charge/refund, provider mutation or Production business-data mutation.
- GitHub-hosted branch/ruleset enforcement is separate from source authority. `main` is governed by the active `rd main protection` ruleset; any protection state that later becomes absent or unobservable must be classified AMBER rather than assumed GREEN.
...
- Production promotion is proposed through a pull request from the accepted Development line to protected `main`; do not bypass `rd main protection` to preserve an obsolete direct-push model.
...
- `.github/workflows/post-recovery-baseline-roadmap-authority.yml` — retained prior-baseline/roadmap authority.
- `scripts/release_authority_documentation_convergence_check.py` — living release/documentation convergence guard.
- `scripts/post_recovery_baseline_roadmap_check.py` — retained prior-baseline/roadmap guard.
