...
for token in [
    ...
    "vehicle_label", "condition_summary", "item.problem", "item.process", "item.result",
    "MAX_PROOF_CARDS = 3",
    'kind: "service"', 'kind: "location"', 'kind: "usecase"',
    "LOCATION_TOWNS", "USE_CASES",
    "build284ContextualProof", "no-real-matching-proof",
    "relevant evidence, not a claim", "Published customer-approved proof", "Real Rosie work",
]:
...
for token in [
    "loadBuild284ContextualProof", "/assets/contextual-proof-v284.js",
    "build284ContextualProofBootstrap", 'data-build282="usecase-conversion"',
    'data-build278="local-seo-depth"', "data-landing-slug", "#rd-landing",
]:
...
for token in ["contextual proof placement", "service", "location", "use-case", "fail-closed", "Build 283", "sample", "Production remains closed"]:
...
for token in ["Build 284 Source Gate", "build284-contextual-proof-placement", "build284_release_check.py", "build283_release_check.py", "build284_http_smoke.sh"]:
...
if "build284_http_smoke.sh" not in dev_gate:
    errors.append("development-source-gate.yml does not syntax-check Build 284 smoke helper")
for token in ["build284_http_smoke.sh", "Build 284 contextual proof"]:
    if token not in cf_gate:
        errors.append(f"cloudflare-development-acceptance.yml missing Build 284 runtime authority: {token}")
...
smoke = read("scripts/build284_http_smoke.sh")
for token in [
    "/assets/contextual-proof-v284.js", "/assets/visual-placeholders.js",
    "no-real-matching-proof", "/api/before_after_gallery_public",
    "/headlight-restoration/", "/tillsonburg-auto-detailing/", "/spring-salt-recovery-detailing/"
]:
    if token not in smoke:
        errors.append(f"build284_http_smoke.sh missing Build 284 token: {token}")

seo_guard = read("scripts/seo_h1_check.py")
for token in ["build284_release_check.py", "build284.exists()", "subprocess.run"]:
