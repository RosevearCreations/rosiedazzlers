...
BASE_URL="${1:-}"
LABEL="${2:-Rosie target}"
SCOPE="${SMOKE_SCOPE:-full}"
[[ -n "$BASE_URL" ]] || { echo "Contextual proof smoke requires a base URL." >&2; exit 2; }
BASE_URL="${BASE_URL%/}"
...
# The browser asset keeps its compatibility filename; the active smoke authority is release-independent.
...
require_text "$tmpdir/location.html" "data-landing-slug=\"tillsonburg-auto-detailing\"" "representative location page"
...
require_text "$tmpdir/usecase.html" "Spring Salt Recovery Car Detailing" "representative use-case page"
...
echo "${LABEL}: contextual proof ${SCOPE} HTTP smoke PASS at ${BASE_URL}"
