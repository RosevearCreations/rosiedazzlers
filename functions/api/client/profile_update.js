...
export async function onRequestPost(context){ const {request,env}=context; try{
 ...
 const patch={
   full_name: cleanText(body.full_name), phone: cleanText(body.phone), address_line1: cleanText(body.address_line1), address_line2: cleanText(body.address_line2), city: cleanText(body.city), province: cleanText(body.province), postal_code: cleanText(body.postal_code), vehicle_notes: cleanText(body.vehicle_notes), notes: cleanText(body.notes), notification_opt_in: toBoolean(body.notification_opt_in), notification_channel: normalizeNotificationChannel(body.notification_channel), detailer_chat_opt_in: toBoolean(body.detailer_chat_opt_in), updated_at: new Date().toISOString()
 };
