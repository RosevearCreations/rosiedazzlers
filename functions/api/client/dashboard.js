...
async function handle(context) {
  ...
  try {
    if (!env?.SUPABASE_URL) {
      return dashboardJson(emptyDashboardPayload({
        code: "configuration_incomplete",
        error: "Customer dashboard is temporarily unavailable because Supabase is not configured."
      }));
    }

    let current = null;
    try {
      current = await getCurrentCustomerSession({ env, request });
    } catch (sessionErr) {
      return dashboardJson(emptyDashboardPayload({
        code: "session_storage_unavailable",
        error: "Customer dashboard sign-in storage is temporarily unavailable.",
        detail: safeErrorMessage(sessionErr)
      }));
    }
    ...
    if (!current?.customer_profile?.id) {
      return dashboardJson(emptyDashboardPayload({
        ...
        error: "Sign in required to view the customer dashboard."
      }), 200, current?.clear_cookie || null);
    }
    ...
    const [bookRes, giftRes, redeemRes, vehicleRes, vehicleMediaRes, tierRes, reviewRes] = await Promise.all([
    ...
    if (!bookRes.ok) {
      return dashboardJson(emptyDashboardPayload({
        code: "booking_history_unavailable",
        error: "Could not load booking history.",
        detail: await safeResponseText(bookRes)
      }));
    }
    if (!giftRes.ok) {
      return dashboardJson(emptyDashboardPayload({
        code: "gift_certificate_history_unavailable",
        error: "Could not load gift certificates.",
        detail: await safeResponseText(giftRes)
      }));
    }
    ...
    const redemptions = Array.isArray(redemptionsAll) ? redemptionsAll.filter((row) => {
      ...
      return (String(gift.purchaser_email || '').toLowerCase() === String(email || '').toLowerCase()) ||
        (String(gift.recipient_email || '').toLowerCase() === String(email || '').toLowerCase());
    }) : [];
    ...
    const vehiclesWithMedia = Array.isArray(vehicles)
      ? vehicles.map((v) => ({ ...v, media: Array.isArray(vehicleMedia) ? vehicleMedia.filter((m) => String(m.vehicle_id) === String(v.id)) : [] }))
      : [];

    return dashboardJson({
      ok: true,
      authenticated: true,
      customer: current.customer_profile,
      tier,
      bookings: Array.isArray(bookings) ? bookings : [],
      vehicles: vehiclesWithMedia,
      vehicle_media: Array.isArray(vehicleMedia) ? vehicleMedia : [],
      gift_certificates: Array.isArray(gifts) ? gifts : [],
    ...
    return dashboardJson(emptyDashboardPayload({
      ...
      error: "Customer dashboard is temporarily unavailable.",
...
function emptyDashboardPayload({ code = "not_authenticated", error = "Sign in required to view the customer dashboard.", detail = null } = {}) {
  return {
    ok: false,
    authenticated: false,
    signed_out: code === "not_authenticated",
    code,
    error,
    detail,
    customer: null,
    ...
    gift_certificates: [],
    redemptions: [],
    gift_summary: summarizeGiftCertificates([], []),
    reviews: []
  };
}
...
function applyCors(headers) {
  ...
  for (const [k, v] of Object.entries(corsHeaders())) if (!out.has(k)) out.set(k, v);
  return out;
}
