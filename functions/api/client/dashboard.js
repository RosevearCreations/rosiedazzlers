...
async function handle(context){
  ...
  try {
    ...
    const vehiclesWithMedia = Array.isArray(vehicles) ? vehicles.map(v => ({ ...v, media: Array.isArray(vehicleMedia) ? vehicleMedia.filter(m => String(m.vehicle_id) === String(v.id)) : [] })) : [];
...
function summarizeGiftCertificates(gifts, redemptions) { const rows = Array.isArray(gifts) ? gifts : []; const used = Array.isArray(redemptions) ? redemptions : []; const now = Date.now(); let active_count = 0; let active_remaining_cents = 0; let expired_count = 0; let redeemed_cents = 0; for (const row of rows) { const remaining = Number(row.remaining_cents || 0); const expiresAt = row.expires_at ? Date.parse(row.expires_at) : null; const expired = Number.isFinite(expiresAt) && expiresAt < now; if (expired) expired_count += 1; if (!expired && String(row.status || '').toLowerCase() !== 'void') { active_count += 1; active_remaining_cents += remaining > 0 ? remaining : 0; } } for (const row of used) redeemed_cents += Number(row.amount_cents || 0); return { active_count, active_remaining_cents, expired_count, redeemed_cents, redemption_count: used.length }; }
