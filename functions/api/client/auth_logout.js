...
export async function onRequestPost(context){ const {request,env}=context; try{
 ...
 return new Response(JSON.stringify({ok:true,message:"Signed out."}),{status:200,headers});
 } catch(err){ let headers=new Headers({"Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store"}); headers=appendSetCookie(headers,buildClearCustomerSessionCookie()); headers=applyCors(headers); return new Response(JSON.stringify({error:err?.message||"Unexpected server error."}),{status:500,headers}); }}
