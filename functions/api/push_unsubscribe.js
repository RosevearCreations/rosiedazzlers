...
import { revokeStaffPushSubscription } from "./_lib/push-subscriptions.js";
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const revoked = await revokeStaffPushSubscription({ env, actor:access.actor, endpoint:body.endpoint });
    return withCors(json({ ok:true, build:270, revoked }));
  } catch (error) {
    return withCors(json({ error:error?.message || "Could not revoke push subscription." },400));
  }
}
