...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const [updatesRes, mediaRes, signoffsRes, workflowRes, socialRes] = await Promise.all([
    ...
    return withCors(json({ ok: true, booking: bookingWithCrew, updates: Array.isArray(updates) ? updates : [], media: Array.isArray(media) ? media : [], signoffs: Array.isArray(signoffs) ? signoffs : [], workflow_events: Array.isArray(workflow_events) ? workflow_events : [], social_posts: Array.isArray(social_posts) ? social_posts : [], social_warning, crew_warning: crewResult.warning || null }));
  } catch (err) {
