...
export async function onRequest(c){
 ...
 try{
  ...
  if(action==='save_financials'){
   const patch={project_classification:clean(b.project_classification,30)||'commercial',expected_revenue_cad:Number(b.expected_revenue_cad||0),actual_revenue_cad:Number(b.actual_revenue_cad||0),profitability_note:clean(b.profitability_note,2000)||null,project_budget_cad:Math.max(0,Number(b.project_budget_cad||0)),target_margin_percent:Math.max(0,Math.min(95,Number(b.target_margin_percent||30))),consent_expires_at:b.consent_expires_at||null,consent_reminder_days:Math.max(1,Math.min(365,Number(b.consent_reminder_days||30))),updated_at:new Date().toISOString(),updated_by_staff_email:email};
   ...
   await rest(c.env,'creative_project_shot_plan_items',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({project_id:projectId,shot_type:clean(b.shot_type,30)||'other',description:clean(b.description,500),required:Boolean(b.required),status:clean(b.status,30)||'planned',consent_required:Boolean(b.consent_required),owner_label:clean(b.owner_label,180)||null,sort_order:Math.max(1,Math.min(9999,Number(b.sort_order||1))),capture_evidence_note:clean(b.capture_evidence_note,1200)||null,safe_note:clean(b.safe_note,1200)||null})});event='shot_plan_updated';safe='Before/after shot-plan item saved.';
  ...
  } else if(action==='queue_consent_reminder'){
   const project=(await rest(c.env,`creative_projects?select=title,consent_status,consent_expires_at,consent_reminder_days&id=eq.${encodeURIComponent(projectId)}&limit=1`))?.[0];
   if(!project?.consent_expires_at)throw new Error('Set a consent expiry date before queuing a reminder.');
   if(['declined','expired'].includes(project.consent_status))throw new Error('Consent reminders cannot be queued for declined or expired consent.');
