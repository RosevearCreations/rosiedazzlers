// Build 451/461/471/481 retained GET-only learning endpoint enriched by owner-approved measurement-lock evidence. Manual refresh.
...
export async function onRequestGet({request,env}){
  ...
  const learning=buildBookingFunnelQuotePricingLearning({funnel:funnel.data||{},quote_rows:Array.isArray(quotes.data?.rows)?quotes.data.rows:[],source_status:sourceStatusMap(funnel,quotes),approval_records:approvalSettings?.booking_quote_experiment_approval_lock||{},generated_at:new Date().toISOString()});
