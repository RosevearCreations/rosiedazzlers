...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (mode === 'slot') {
      ...
      const res = await fetch(`${env.SUPABASE_URL}/rest/v1/slot_blocks?on_conflict=blocked_date,slot`, { method: 'POST', headers, body: JSON.stringify([{ blocked_date, slot, reason }]) });
    ...
    const reason = cleanNullableText(body.reason);
    if (!blocked_date) return withCors(json({ error: 'Valid blocked_date is required.' }, 400));
    const res = await fetch(`${env.SUPABASE_URL}/rest/v1/date_blocks?on_conflict=blocked_date`, { method: 'POST', headers, body: JSON.stringify([{ blocked_date, reason }]) });
    const payload = await res.json().catch(() => []);
...
function normalizeDateRow(row) { return row ? { ...row, block_date: row.blocked_date || null, is_closed: true, updated_at: row.created_at || null } : null; }
function normalizeSlotRow(row) { return row ? { ...row, block_date: row.blocked_date || null, slot_code: row.slot || null, is_blocked: true, updated_at: row.created_at || null } : null; }
function actorShape(actor) { return { id: actor?.id || null, full_name: actor?.full_name || null, email: actor?.email || null, role_code: actor?.role_code || null }; }
function cleanDate(value) { const s = cleanText(value); if (!s) return null; return /^\d{4}-\d{2}-\d{2}$/.test(s) ? s : null; }
function cleanNullableText(value) { const s = cleanText(value); return s || null; }
