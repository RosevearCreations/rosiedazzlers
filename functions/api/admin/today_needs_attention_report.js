...
async function handle({request,env}){
  try{
    ...
    const taskIndex=indexTasks(loaded.attention_tasks||[]);
    const visible=[];
    for(const row of dedup){const task=taskIndex.get(`${row.source_type}|${row.source_key}`)||null;if(shouldHideByTask(task,now))continue;visible.push({...row,attention_task:task?taskSummary(task):null});}
    for(const task of latestManualTasks(loaded.attention_tasks||[])){
      if(shouldHideByTask(task,now))continue;
      visible.push({urgency:task.urgency||'normal',title:task.title||'Owner task',detail:task.detail||'Manual owner task.',target:task.target_url||'/admin.html',booking_id:task.booking_id||null,source_type:task.source_type||'manual',source_key:task.source_key||task.id,attention_task:taskSummary(task)});
    }
    const rank={urgent:0,high:1,normal:2,low:3};visible.sort((a,b)=>(rank[a.urgency]-rank[b.urgency])||new Date(a.attention_task?.updated_at||0)-new Date(b.attention_task?.updated_at||0));
    return withCors(json({ok:true,generated_at:now.toISOString(),today,counts:{total:visible.length,urgent:visible.filter(i=>i.urgency==='urgent').length,high:visible.filter(i=>i.urgency==='high').length,normal:visible.filter(i=>i.urgency==='normal').length},items:visible.slice(0,250),warnings,migrations_ready:warnings.length===0,owner_task_controls_ready:!(warnings||[]).some(w=>String(w).startsWith('attention_tasks:'))}));
  }catch(err){return withCors(json({ok:false,error:err?.message||'Could not build Today Needs Attention report.'},500));}
}
function indexTasks(rows){const map=new Map();for(const row of rows||[]){const key=`${row.source_type||'generated'}|${row.source_key||''}`;if(!key.endsWith('|')&&!map.has(key))map.set(key,row);}return map;}
function latestManualTasks(rows){const out=[];const seen=new Set();for(const row of rows||[]){if(String(row.source_type||'')!=='manual')continue;const key=row.source_key||row.id;if(key&& !seen.has(key)){seen.add(key);out.push(row);}}return out;}
...
function taskSummary(row){return {id:row.id,status:row.status,assigned_to_staff_name:row.assigned_to_staff_name||null,snoozed_until:row.snoozed_until||null,resolution_note:row.resolution_note||null,updated_at:row.updated_at||null};}
