...
async function handle({request,env}){
  try{
    ...
    for(const task of latestManualTasks(loaded.attention_tasks||[])){
      ...
      visible.push({urgency:overdue?'urgent':(task.urgency||'normal'),title:overdue?`Overdue: ${task.title||'Owner task'}`:(task.title||'Owner task'),detail:task.detail||'Manual owner task.',target:task.target_url||'/admin-today.html',booking_id:task.booking_id||null,source_type:task.source_type||'manual',source_key:task.source_key||task.id,attention_task:taskSummary(task)});
    }
