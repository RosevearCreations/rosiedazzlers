...
async function handle({request,env}){
  try{
    ...
    const queries={
      ...
      balances:`final_balance_payment_requests?select=id,booking_id,customer_name,status,amount_cents,payment_url,created_at&order=created_at.desc&limit=150`,
    ...
    for(const row of loaded.notifications||[]){if(["failed","retry"].includes(clean(row.status)))items.push(item("high","Notification failed",`${row.event_type||"notification"}: ${row.last_error||"retry needed"}`,`/admin-notifications.html`,row.booking_id));}
    for(const row of loaded.balances||[]){if(!/paid|cancel/i.test(String(row.status||"")))items.push(item("normal","Balance request open",`${row.customer_name||"Customer"} · ${money(row.amount_cents)} · ${row.status||"draft"}`,`/admin-payments.html`,row.booking_id));}
