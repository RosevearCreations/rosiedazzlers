...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const itemRes = await fetch(`${env.SUPABASE_URL}/rest/v1/catalog_inventory_items?select=id,item_key,name,preferred_vendor,reorder_qty,cost_cents,amazon_url&item_key=eq.${encodeURIComponent(item_key)}&limit=1`, { headers: { apikey: env.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`, Accept: 'application/json' } });
