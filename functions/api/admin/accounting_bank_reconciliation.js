...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const [snapshot, reconciliations] = await Promise.all([buildBankReconciliationSnapshot(env,{month,year,accountCode}),listBankReconciliations(env,{month,year,accountCode})]);
...
export async function onRequestPost({ request, env }) {
  try {
    const access = await requireStaffAccess({ request, env, capability:null, allowLegacyAdminFallback:false });
    if (!access.ok) return withCors(access.response);
    const actionAccess = requireActionAccess(access.actor, 'finance.reconcile');
    if (!actionAccess.ok) return withCors(actionAccess.response);
    const body = await request.json().catch(()=>({}));
    const saved = await saveBankReconciliation(env, body, { name: access.actor?.full_name || access.actor?.email || null, staffUserId: access.actor?.id || null });
