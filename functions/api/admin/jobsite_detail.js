...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const [bookingRes, intakeRes, timeRes, updatesRes, mediaRes, signoffRes, commentsRes, annRes] = await Promise.all([
      fetch(`${env.SUPABASE_URL}/rest/v1/bookings?select=id,created_at,service_date,start_slot,status,job_status,customer_name,customer_email,customer_phone,package_code,vehicle_size,price_total_cents,deposit_cents,notes,progress_enabled,progress_token,assigned_to,assigned_staff_user_id,assigned_staff_email,assigned_staff_name,customer_profile_id,customer_tier_code&id=eq.${encodeURIComponent(booking_id)}&limit=1`, { headers }),
      fetch(`${env.SUPABASE_URL}/rest/v1/jobsite_intake?select=id,booking_id,pre_existing_condition,valuables,pre_job_checklist,owner_notes,acknowledgement_notes,intake_complete,updated_by_staff_user_id,updated_by_staff_name,created_at&booking_id=eq.${encodeURIComponent(booking_id)}&limit=1`, { headers }),
      ...
      fetch(`${env.SUPABASE_URL}/rest/v1/job_signoffs?select=id,booking_id,customer_name,signature_data_url,approval_notes,signed_at,updated_by_staff_user_id,updated_by_staff_name,created_at&booking_id=eq.${encodeURIComponent(booking_id)}&limit=1`, { headers }),
      fetch(`${env.SUPABASE_URL}/rest/v1/progress_comments?select=id,booking_id,parent_type,parent_id,author_type,author_name,author_email,message,visibility,thread_status,moderated_at,moderated_by_name,moderation_reason,created_at&booking_id=eq.${encodeURIComponent(booking_id)}&order=created_at.asc`, { headers }).catch(() => null),
      fetch(`${env.SUPABASE_URL}/rest/v1/observation_annotations?select=id,booking_id,media_id,x_percent,y_percent,title,note,visibility,category,severity,pin_color,created_by_type,created_by_name,thread_status,moderated_at,moderated_by_name,moderation_reason,created_at&booking_id=eq.${encodeURIComponent(booking_id)}&order=created_at.asc`, { headers }).catch(() => null)
    ]);
