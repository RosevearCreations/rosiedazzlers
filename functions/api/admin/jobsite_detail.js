...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const [bookingRes, intakeRes, timeRes, updatesRes, mediaRes, signoffRes, commentsRes, annRes] = await Promise.all([
      ...
      fetch(`${env.SUPABASE_URL}/rest/v1/observation_annotations?select=id,booking_id,media_id,x_percent,y_percent,title,note,visibility,category,severity,pin_color,created_by_type,created_by_name,created_at&booking_id=eq.${encodeURIComponent(booking_id)}&order=created_at.asc`, { headers }).catch(() => null)
    ]);
