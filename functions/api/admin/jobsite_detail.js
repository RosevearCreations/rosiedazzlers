...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const [bookingRes, intakeRes, timeRes, updatesRes, mediaRes, signoffRes, commentsRes] = await Promise.all([
    ...
    const [bookingRows,intakeRows,timeRows,updateRows,mediaRows,signoffRows,commentRows] = await Promise.all([bookingRes.json().catch(()=>[]),intakeRes.json().catch(()=>[]),timeRes.json().catch(()=>[]),updatesRes.json().catch(()=>[]),mediaRes.json().catch(()=>[]),signoffRes.json().catch(()=>[]), commentsRes && commentsRes.ok ? commentsRes.json().catch(()=>[]) : []]);
