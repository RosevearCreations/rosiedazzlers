  ...
  try {
    ...
    media = media
      ...
      .filter((row) => {
        ...
        return row.usage_contexts.includes(usageContext) || row.group_key === usageContext;
      })
      .filter((row) => {
        if (!groupKey) return true;
        return row.group_key === groupKey;
      })
      .filter((row) => {
        if (!mediaType) return true;
        return row.media_type === mediaType || (mediaType === "image" && ["photo", "image"].includes(row.media_type));
      })
      .sort((a, b) => {
        return (
...
async function loadMediaLibraryTable(env, warnings) {
  ...
  try {
    ...
    if (!res.ok) {
      warnings.push(`app_media_library table not available: ${await safeText(res)}`);
      return [];
    }
...
async function loadMediaLibrarySetting(env, warnings) {
  ...
  try {
    ...
    if (!res.ok) {
      warnings.push(`media_library app setting not available: ${await safeText(res)}`);
      return [];
    }
...
async function safeText(response) {
  try {
    ...
    return `${response.status || "unknown"} ${response.statusText || ""}`.trim();
  }
}
