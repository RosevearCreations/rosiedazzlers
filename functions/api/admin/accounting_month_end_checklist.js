...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const checklist = normalizeRow(row);
    const closure = await buildMonthEndClosureSnapshot(env, { month, year, checklist });

    return withCors(json({ ok: true, month, year, month_start: monthStart, checklist, closure }));
  } catch (err) {
