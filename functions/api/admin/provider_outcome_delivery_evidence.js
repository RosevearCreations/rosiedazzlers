...
// Read-only refresh of retained provider evidence. No provider mutation is performed.
...
export async function onRequestGet({ request, env }) {
  ...
  const evidence = buildProviderOutcomeDeliveryEvidence({ closure: payload.closure, generated_at: generatedAt });
  return json({
    ok: true,
    build: 436,
    authority: "provider_outcome_delivery_evidence_closure",
...
function json(value, status = 200) {
  return new Response(JSON.stringify(value), {
    ...
    headers: {
      ...
      "X-Rosie-Provider-Evidence": "build-436-read-only"
    }
  });
}
