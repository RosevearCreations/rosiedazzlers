// Build 362 — list quote/proposal drafts with delivery, immutable acceptance, and deposit tracking.
import { requireStaffAccess, json, serviceHeaders, cleanText, isUuid, methodNotAllowed } from "../_lib/staff-auth.js";

const DRAFT_SELECT = [
  "id", "lead_id", "booking_id", "title", "status", "body", "pricing_note", "internal_note", "customer_name", "customer_email", "source",
  "follow_up_at", "sent_at", "delivery_status", "delivery_to_email", "delivery_subject", "delivered_at", "acceptance_status",
  "structured_terms", "structured_terms_hash", "structured_terms_created_at", "terms_expires_at",
  "accepted_terms", "accepted_terms_hash", "accepted_terms_at", "accepted_at", "declined_at", "responded_at", "customer_response_note",
  "deposit_request_status", "deposit_requested_at", "deposit_paid_at", "final_booking_confirmed_at", "final_booking_id", "latest_deposit_payment_request_id",
  "created_by_staff_user_id", "updated_by_staff_user_id", "created_at", "updated_at"
].join(",");
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!hasSupabaseConfig(env)) return withCors(json({ ok: true, table_ready: false, drafts: [], warning: "Server configuration is incomplete. Quote/proposal drafts cannot load yet.", migration_hint: "Apply the quote migrations through Build 362 and confirm Supabase service env vars." }));
    const drafts = await loadDrafts(env, body);
    return withCors(json({ ok: true, table_ready: true, drafts }));
  } catch (err) {
    return withCors(json({ ok: true, table_ready: false, drafts: [], warning: err?.message || "Quote/proposal draft table is not ready.", migration_hint: "Apply sql/2026-09-09_build362_quote_booking_acceptance.sql after the earlier quote migrations." }));
  }
}
...
async function loadDrafts(env, body) {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/quote_proposal_drafts?${params.toString()}`, { headers: serviceHeaders(env) });
...
function queryBody(request) { return Object.fromEntries(new URL(request.url).searchParams.entries()); }
function normalizeStatusFilter(value) { return String(value || "all").trim().toLowerCase().replace(/[^a-z0-9_]+/g, "_") || "all"; }
function escapeLike(value) { return encodeURIComponent(String(value || "").replace(/[,*()]/g, " ").trim().slice(0, 80)); }
...
function hasSupabaseConfig(env) { return !!(env?.SUPABASE_URL && getSupabaseServiceRoleKey(env)); }
