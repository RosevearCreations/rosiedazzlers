...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!isAllowedKey(key)) return json({ ok:false, error:"Only approved public asset folders are allowed: packages/, landing_pages/, landing-pages/, CarPhotos/, gallery/, videos/, brand/, addons/, and products/." }, 400);
    const maxBytes = /^videos\//i.test(key) ? 250 * 1024 * 1024 : 20 * 1024 * 1024;
    if (Number(file.size || 0) > maxBytes) return json({ ok:false, error:`File exceeds the ${Math.round(maxBytes/1024/1024)} MB admin upload limit.` }, 413);
    ...
    await bucket.put(key, bytes, { httpMetadata: { contentType }, customMetadata: { uploaded_by: access.actor?.email || "staff", build: "253" } });
    const publicBase = String(env.PUBLIC_ASSET_BASE_URL || env.ASSETS_PUBLIC_BASE_URL || "https://assets.rosiedazzlers.ca/").replace(/\/?$/, "/");
    return json({ ok:true, build:"253", r2_key:key, url: publicBase + key.split("/").map(encodeURIComponent).join("/"), content_type:contentType, size_bytes:bytes.byteLength, dimensions:dims, next_step:"Run Media Health again and confirm the asset is public." });
