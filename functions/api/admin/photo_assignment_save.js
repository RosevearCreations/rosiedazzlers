...
// Build 391: public-only assignment authority, explicit pair metadata, and safe reset/unassign semantics.
import { requireStaffAccess, serviceHeaders, json } from '../_lib/staff-auth.js';
import { safeText } from '../_lib/photo-library.js';
import { PHOTO_STUDIO_BUILD, beforeAfterPlacement, loadAssignablePublicPhoto } from '../_lib/photo-studio-safety.js';

const BUILD=PHOTO_STUDIO_BUILD;

function migrationResponse(detail=''){
  return json({ok:false,migration_required:true,migration:'sql/2026-08-12_build253_photo_management_studio.sql',error:'Photo assignments are not ready. Apply the Build 253 photo-management migration, then retry.',detail:String(detail||'').slice(0,500)},409);
}
...
async function verifyDistinctPairPhoto(env,{targetKey,targetType,mediaId,headers}){
  ...
  if(counterpart&&String(counterpart.media_id||'')===String(mediaId||''))return {ok:false,conflict:true,counterpart_key:counterpartKey,counterpart_label:counterpart.target_label||counterpartKey,error:'Before and After must use two different photos. Select a different image for the paired slot.'};
...
export async function onRequestPost({request,env}){
  ...
  try{
    ...
    const pair=beforeAfterPlacement(targetKey,targetType);
    const now=new Date().toISOString();
    const action=String(body.action||'assign').trim().toLowerCase();
    if(action==='remove'||action==='reset'||action==='unassign'){
      const response=await fetch(`${env.SUPABASE_URL}/rest/v1/app_media_assignments?target_key=eq.${encodeURIComponent(targetKey)}`,{method:'PATCH',headers:{...serviceHeaders(env),Prefer:'return=minimal'},body:JSON.stringify({is_active:false,updated_at:now,updated_by:access.actor?.email||'staff'})});
      ...
      return json({ok:true,build:BUILD,removed:true,reset_to_default:true,asset_deleted:false,target_key:targetKey,multi_placement_supported:true,...pair});
    }
    if(action!=='assign')return json({ok:false,error:'Unsupported photo assignment action.'},400);

    const mediaId=safeText(body.media_id,80);
    const mediaCheck=await loadAssignablePublicPhoto(env,mediaId);
    if(!mediaCheck.ok)return json({ok:false,build:BUILD,private_media_blocked:mediaCheck.private_media_blocked===true,error:mediaCheck.error},mediaCheck.status||400);
    ...
    const payload={target_key:targetKey,media_id:mediaId,target_label:safeText(body.target_label,240)||targetKey,target_type:targetType,page_path:safeText(body.page_path,300)||null,component_key:safeText(body.component_key,160)||null,variant:safeText(body.variant,80)||null,alt_override:safeText(body.alt_override,300)||null,title_override:safeText(body.title_override,240)||null,caption_override:safeText(body.caption_override,1000)||null,is_active:true,updated_at:now,updated_by:access.actor?.email||'staff'};
    const response=await fetch(`${env.SUPABASE_URL}/rest/v1/app_media_assignments?on_conflict=target_key`,{method:'POST',headers:{...headers,Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify(payload)});
    ...
    const rows=await response.json().catch(()=>[]);
    return json({ok:true,build:BUILD,assignment:Array.isArray(rows)?rows[0]||null:null,public_media_verified:true,multi_placement_supported:true,pair_counterpart_key:pairCheck.counterpart_key||'',pair_counterpart:pairCheck.counterpart||null,...pair});
  }catch(err){return json({ok:false,build:BUILD,error:err?.message||'Could not save photo assignment.'},500);}
}
