...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const acceptance = await buildFinanceCloseAcceptanceSnapshot(env, { month, year });
    return withCors(json({ ok: true, acceptance }));
  } catch (err) {
    return withCors(json({ error: err?.message || "Could not calculate Finance close acceptance." }, 500));
  }
}
