...
import { buildRetentionMaintenanceFleetOperationalPilot } from "../_lib/retention-maintenance-fleet-operational-pilot.js";
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    if (!hasSupabaseConfig(env)) {
      return withCors(json({ ok: false, error: "Operational pilot service configuration is incomplete." }, 500));
    }
    ...
    const commercialActivation = buildCommercialActivationState({
    ...
    const operationalPilot = buildRetentionMaintenanceFleetOperationalPilot({
      commercial_activation: commercialActivation
    });

    return withCors(json({
      ok: true,
      operational_pilot: operationalPilot,
      source_authorities: {
        commercial_activation: "/api/admin/commercial_activation",
    ...
    console.error("Retention / maintenance / fleet operational pilot overview failed.", {
      message: err?.message || "Unknown error"
    });
    return withCors(json({ ok: false, error: "Could not load the operational pilot overview." }, 500));
  }
}
...
async function fetchMaintenanceInterestRows(env) {
  ...
  if (!res.ok) throw new Error(`operational_pilot_maintenance_read_failed_${res.status}`);
...
async function fetchFleetLeadRows(env) {
  ...
  if (!res.ok) throw new Error(`operational_pilot_fleet_read_failed_${res.status}`);
  const rows = await res.json().catch(() => []);
  return Array.isArray(rows) ? rows : [];
}
