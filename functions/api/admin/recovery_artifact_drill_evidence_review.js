...
export async function onRequestGet({request,env}){
 ...
 const review=buildRecoveryArtifactDrillEvidenceReview({closure:payload?.evidence||{},generated_at:generatedAt});
 return json({ok:response.ok&&Boolean(payload?.evidence),build:447,authority:"recovery_artifact_drill_evidence_review",generated_at:generatedAt,retained_authority:payload?.authority||"backup_recovery_evidence_closure",review},response.ok&&payload?.evidence?200:503);
}
