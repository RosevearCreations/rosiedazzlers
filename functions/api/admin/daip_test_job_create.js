...
export async function onRequestPost({request, env}) {
  try {
    ...
    if (!/^RD-TEST-BOOKING-[A-Z0-9-]{3,80}$/.test(testBookingReference)) return daipError('Enter a DAIP-only test reference such as RD-TEST-BOOKING-DEMO-01. Do not use any booking UUID or customer record.', 400);
    if (!safeLabel) return daipError('Enter a short safe internal-test label with no customer name, address, VIN, or phone number.', 400);
    if (!jobDate) return daipError('Choose a valid job date.', 400);
    if (!isInternalTestAcknowledged(body)) return daipError('Confirm INTERNAL TEST ONLY, no customer data, and no public export before creating a DAIP test job.', 400);
    ...
    return withCors(json({ok:true, build:DAIP_BUILD, job:{id:job.id,job_code:job.job_code,safe_label:job.safe_label,status:job.status,job_date:job.job_date}, warning:taskWarning}));
