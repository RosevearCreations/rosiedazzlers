...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const payload = { item_key: String(body?.item_key || '').trim(), item_type: String(body?.item_type || '').trim().toLowerCase(), name: String(body?.name || '').trim(), category: String(body?.category || '').trim() || null, image_url: String(body?.image_url || '').trim() || null, amazon_url: String(body?.amazon_url || '').trim() || null, qty_on_hand: Number(body?.qty_on_hand || 0), reorder_point: Number(body?.reorder_point || 0), reorder_qty: Number(body?.reorder_qty || 0), unit_label: String(body?.unit_label || '').trim() || null, rating_value: body?.rating_value == null || body?.rating_value === '' ? null : Number(body.rating_value), rating_count: Number(body?.rating_count || 0), preferred_vendor: String(body?.preferred_vendor || '').trim() || null, notes: String(body?.notes || '').trim() || null, is_public: body?.is_public !== false, is_active: body?.is_active !== false, updated_at: new Date().toISOString() };
