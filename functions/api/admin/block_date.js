...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const res = await fetch(`${env.SUPABASE_URL}/rest/v1/date_blocks?on_conflict=blocked_date`, { method: "POST", headers: { ...serviceHeaders(env), Accept: "application/json", Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify([{ blocked_date, reason }]) });
