...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!["AM", "PM"].includes(slot)) return json({ error: "Invalid slot. Use AM or PM." }, 400);
    const deleteRes = await fetch(`${env.SUPABASE_URL}/rest/v1/slot_blocks?blocked_date=eq.${encodeURIComponent(blocked_date)}&slot=eq.${encodeURIComponent(slot)}`, { method: "DELETE", headers: { ...serviceHeaders(env), Prefer: "return=representation" } });
    if (!deleteRes.ok) return json({ error: `Could not unblock slot. ${await deleteRes.text()}` }, 500);
    const rows = await deleteRes.json().catch(() => []);
    const removed = Array.isArray(rows) ? rows[0] || null : null;
    return json({ ok: true, message: "Slot block removed.", actor: { id: access.actor.id || null, full_name: access.actor.full_name || null, email: access.actor.email || null, role_code: access.actor.role_code || null }, removed: removed || { blocked_date, slot } });
  } catch (err) {
