  ...
  try {
    ...
    return withCors(
      json({
        ...
        date_blocks: Array.isArray(dateBlockRows) ? dateBlockRows.map((row) => ({ ...row, block_date: row.blocked_date || null, is_closed: true, updated_at: row.created_at || null })) : [],
        slot_blocks: Array.isArray(slotBlockRows) ? slotBlockRows.map((row) => ({ ...row, block_date: row.blocked_date || null, slot_code: row.slot || null, is_blocked: true, updated_at: row.created_at || null })) : []
      })
    );
  } catch (err) {
    return withCors(
