import { requireStaffSessionOrThrow } from "../_lib/staff-auth.js";

export async function onRequestPost(context) {
  const { request, env } = context;
  try {
    const actor = await requireStaffSessionOrThrow(context, { allowLegacyFallback: false });
    ...
    const insertRes = await fetch(`${env.SUPABASE_URL}/rest/v1/job_updates`, {
      ...
      body: JSON.stringify([{ booking_id: bookingId, created_by: actor.full_name || actor.email || 'Staff', note: note.slice(0, 4000), visibility }])
    });
    ...
    await fetch(`${env.SUPABASE_URL}/rest/v1/booking_events`, {
      method: 'POST',
      headers,
      body: JSON.stringify([{ booking_id: bookingId, event_type: visibility === 'internal' ? 'internal_note_posted' : 'detailer_update_posted', event_note: note.slice(0, 250), actor_name: actor.full_name || actor.email || 'Staff', payload: { visibility, actor_id: actor.id || null } }])
    }).catch(() => null);
