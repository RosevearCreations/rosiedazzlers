...
export async function createCustomerSession({ env, customerProfile, request, days = DEFAULT_SESSION_DAYS }) {
  ...
  if (!customerProfile || !customerProfile.id) {
    throw new Error("createCustomerSession requires a valid customer profile.");
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(`Could not create customer session. ${text}`);
  }
...
export async function getCurrentCustomerSession({ env, request }) {
  ...
  if (!rawToken) {
    return { ok: true, session: null, customer_profile: null, needs_rotation: false };
  }
  ...
  const res = await fetch(
    `${env.SUPABASE_URL}/rest/v1/customer_auth_sessions` +
      ...
      `customer_profile:customer_profiles!customer_auth_sessions_customer_profile_id_fkey(id,created_at,updated_at,email,full_name,phone,tier_code,notes,address_line1,address_line2,city,province,postal_code,vehicle_notes,is_active)` +
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Could not load current customer session. ${text}`);
  }
  ...
  if (!session || session.revoked_at) {
    return { ok: true, session: null, customer_profile: null, needs_rotation: false, clear_cookie: buildClearCustomerSessionCookie() };
  }
  ...
  if (!customerProfile || customerProfile.is_active !== true) {
    return { ok: true, session: null, customer_profile: null, needs_rotation: false, clear_cookie: buildClearCustomerSessionCookie() };
  }
  ...
  return {
    ...
    needs_rotation: shouldRotateSession(session),
    raw_token: rawToken
  };
}
...
export async function rotateCustomerSession({ env, request, currentSession, customerProfile }) {
  ...
  if (!currentSession || !currentSession.id || !customerProfile || !customerProfile.id) {
    throw new Error("rotateCustomerSession requires current session and customer profile.");
  }
...
async function makeOpaqueSessionToken(env, customerProfileId) {
  ...
  const base = `${customerProfileId}.${Date.now()}.${randomHex}.${env.CUSTOMER_SESSION_SECRET}`;
...
function shouldRotateSession(session) {
  ...
  return (Date.now() - updatedAt) >= SESSION_ROTATE_AFTER_HOURS * 60 * 60 * 1000;
}
...
function assertSessionEnv(env) {
  ...
  if (!env?.CUSTOMER_SESSION_SECRET) throw new Error("Missing CUSTOMER_SESSION_SECRET.");
}
