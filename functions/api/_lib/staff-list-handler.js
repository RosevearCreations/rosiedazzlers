// Build 350 — canonical Staff & Access list authority shared by legacy and /admin routes.
// Keep identity/security fields server-side; both public function routes return the same normalized profile contract.
import { requireStaffAccess, serviceHeaders, json } from "./staff-auth.js";
...
export async function handleStaffListRequest(context) {
...
async function loadStaffRows(env, warnings) {
  ...
  // Optional payroll/profile fields may lag the core staff schema during a deploy.
  // Fall back to the core access profile rather than hiding the staff list.
