...
async function run(){const btn=$('#runScan');btn.disabled=true;btn.textContent='Scanning…';results=[];assetCache=new Map();note('Scanning critical public/admin routes and their local assets…');try{const data=await fetch('/data/build247_ui_health_routes.json',{cache:'no-store'}).then((r)=>r.json());routes=Array.isArray(data.routes)?data.routes:[];for(let i=0;i<routes.length;i++){note(`Scanning ${i+1}/${routes.length}: ${routes[i].label}`);results.push(await scanRoute(routes[i]));render();}}catch(error){note(error.message||'Could not run the UI health scan.','bad');}finally{btn.disabled=false;btn.textContent='Run full UI & SEO scan';}}
