...
function normalizeAddons(rows) {
  return (Array.isArray(rows) ? rows : []).map((addon) => {
    ...
    return {
      ...
      requires_package_codes_any: (Array.isArray(addon?.requires_package_codes_any) ? addon.requires_package_codes_any : []).map((row) => String(row || "").trim()).filter(Boolean),
      requirement_note: String(addon?.requirement_note || "").trim() || null,
